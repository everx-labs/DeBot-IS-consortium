...
abstract contract Debot {
    ...
    /// @notice Flags for sending external inbound messages:
    /// @notice Override msg creation time with `time` argument
    uint8 constant OVERRIDE_TS = 1;
    /// @notice Override msg expiration period with `expire` argument
    uint8 constant OVERRIDE_EXPT = 2;
    /// @notice Send message asynchronously (without waiting for txn completion)
    ...
    function _abiver(uint8 maj, uint8 min) internal pure inline returns (uint8) {
        return (min << 4 | maj);
    }
}